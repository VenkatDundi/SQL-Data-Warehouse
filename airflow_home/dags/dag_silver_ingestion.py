...
from etl_python.silver.load_silver import load_to_silver # type: ignore
...
with DAG(
    ...
    def task_load_silver_layer(ti, **kwargs):
        start = time()
        result = load_to_silver()        # Silver layer stored procedure
        if result == True:
            ti.xcom_push(key='task_update', value=f'Ingestion to Silver layer completed successfully') 
            ti.xcom_push(key = 'state', value = 'success')
            end = time()
            ti.xcom_push(key = 'duration', value = round((end - start), 4))
        else:
            ti.xcom_push(key='task_update', value=f'Ingestion to Silver layer had failed!')
    ...
    task_load_silver = PythonOperator(                        
        ...
        task_id = 'load_silver',
        python_callable=task_load_silver_layer
    )
    ...
    task_tablerows_Initial = SQLExecuteQueryOperator(
        ...
        sql = """ EXEC DataWarehouse.silver.sp_PrintTableRowCounts """,
        do_xcom_push = True
    )
